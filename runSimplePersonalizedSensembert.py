...
class personalziedWSD:
    ...
    def loadBERT(self, pathName):
        ...
        model = BertModel.from_pretrained(pathName,
    ...
    def loadSenseDistribution(self, dataset):
            ...
            for lemma in senseCounts[author]:
                ...
                for sense in wordnetSenses:
                    ...
                    if(sense in senseCounts[author][lemma]):
                        senseDistr[author][lemma][sense] = float(senseCounts[author][lemma][sense])/float(lemmaCounts[author][lemma])
                    else:
                        senseDistr[author][lemma][sense] = 0.0
                ordered_distr = {k: v for k, v in sorted(senseDistr[author][lemma].items(), key=lambda item: item[1], reverse=True)}
                currRank = 1
        ...
        for lemma in datasetSenseCounts:
            ...
            for sense in datasetSenseCounts[lemma]:
                datasetSenseDistr[lemma][sense] = float(datasetSenseCounts[lemma][sense])/float(datasetLemmaCounts[lemma])
            ordered_dataset_distr = {k: v for k, v in sorted(datasetSenseDistr[lemma].items(), key=lambda item: item[1], reverse=True)}
            currRank = 1
    ...
    def getAuthorPredomSense(self):
        ...
        for authorID in senseCounts:
            ...
            for lemma in senseCounts[authorID]:
                ...
                if(tiedSenses):
                    predomSenses[authorID][lemma] = maxSense #Assign the first sense ('random')
                else:              
    ...
    def GetBERTTokenEmbedding(self,text, device='cpu'):
        ...
        tokenIndices = self.findTargetInBertTokeinzedText(tokenized_text, numNonWhite) #Returns array indices that point to target or target parts if tokenizer split

        idx = 0

        enc =[tokenizer.encode(x, add_special_tokens=False) for x in marked_text.split()]
        ...
        with torch.no_grad():
            ...
            for tokenIndex in tokenIndices: 
                ...
                if(targetTokenEmbedding == None):
                    ...
                    targetTokenEmbedding = torch.add(targetTokenEmbedding,currTokenEmbedding)
    ...
    def findSimilarSenses(self, tokenEmbedding_in, senseEmbeddings, lemma):
        ...
        maxSimSense = None
        shortList = self.getSensesOfLemma(lemma) #Jsut get the senses of the target lemma
       
        for currSense in shortList:
                currSenseEmbedding = senseEmbeddings[currSense]
                
                currSim = dot(tokenEmbedding, currSenseEmbedding)/(norm(tokenEmbedding)*norm(currSenseEmbedding)) #Cosine similarity
    ...
    def predictSense(self, text, lemma):
        ...
        formatedTargetEmbedding = torch.cat((targetTokenEmbedding,targetTokenEmbedding),dim=0)
        #print(len(targetTokenEmbedding))   

        mostSimilarSense, mostSimilarSenseVal = self.findSimilarSenses(formatedTargetEmbedding, self.senseEmbeddings, lemma)
    ...
    def predictSenseUsingAuthorPredomSense(self, text, lemma, authorID, topX):
        ...
        for currSense in wordnetSenses:
            ...
            if(currSense in self.senseEmbeddings):
                ...
                currSim = dot(formatedTargetEmbedding, currSenseEmbedding)/(norm(formatedTargetEmbedding)*norm(currSenseEmbedding)) #Cosine similarity
        ...
        predSensembertRanks = {}
        orderedSensembert = [k for k, v in sorted(senseScores.items(), key=lambda item: item[1], reverse=True)]
        senseRank = 1
...
if __name__ == '__main__':
    ...
    myPersonalizedModel.loadBERT('bert-large-cased') #Should automatically download the model
    ...
    myPersonalizedModel.loadSenseEmbertEmbeddingsText('data/sensembert_EN_kb.txt') #Run this at least once to generate json
    ...
    example_topX = 3
    authorPredomPredictedSense = myPersonalizedModel.predictSenseUsingAuthorPredomSense(example_sent, example_lemma, example_authorID, example_topX)
    print("prediction using author's predominant sense with senseEmbert: ", authorPredomPredictedSense)
